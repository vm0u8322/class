...
2. **準備範例檔案**：為了獲得最佳測試體驗，建議使用本專案 `example/` 資料夾內的講義、照片與錄音檔。
   - **重要提示**：由於 Git 與 ZIP 下載都不會保留檔案原始的系統建立日期（導致無法模擬「時間配對」功能），請在**取得專案（Git Clone 或下載 ZIP）**後，還原範例檔案的正確教學時間戳記：
     - **快速執行**：在 `tools/restore_example_timestamps.ps1` 檔案上按 **滑鼠右鍵**，選擇 **「使用 PowerShell 執行」**。
     - **指令執行**：若有權限限制，請在終端機輸入 `PowerShell -ExecutionPolicy Bypass -File tools/restore_example_timestamps.ps1`。
3. **上傳並自動整理**：將範例檔案拖入 MochiClass 介面，系統將自動根據時間與 OCR 內容，將檔案分類至正確課程。
